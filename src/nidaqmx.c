...
int32 DAQmxGetErrorString(int32 errorCode, char errorString[],
                          uInt32 bufferSize)
{
  return 0;
}
...
int32 DAQmxClearTask(TaskHandle taskHandle) { return 0; }
...
int32 DAQmxGetDevAIVoltageRngs(const char device[], float64 *data,
...
int32 DAQmxCfgDigEdgeStartTrig(TaskHandle taskHandle,
                               const char triggerSource[], int32 triggerEdge)
{
  return 0;
}
...
int32 DAQmxStartTask(TaskHandle taskHandle) { return 0; }

int32 DAQmxStopTask(TaskHandle taskHandle) { return 0; }
...
int32 DAQmxResetDevice(const char deviceName[]) { return 0; }
                         ...
                         bool32 *reserved)
{
  ...
  memset(readArray, (uint8_t)(rand() % 256), arraySizeInSamps * sizeof(float64));
  return 0;
}
